...
import { SKU_ID_MAP, PETIT_SKU_CODE_MAP, SKU_ID_PACK_QTY, SKIP_TO_FLOEM, FORCE_STICKER } from "./skuMapping";
...
// ── 엑셀 생성 ──
// ══════════════════════════════════════════════
// 미매칭 항목만 엑셀로 내보내기 (SKU ID, SKU 이름, 바코드 + 빈 품목코드/개입수 열)
function exportUnmatched(processed, brand) {
  const unmatched = processed.filter(x => !x.code && !x.displayName);
  if (unmatched.length === 0) { alert("미매칭 항목이 없습니다!"); return; }
  const wsData = [["SKU ID","SKU 이름","바코드","품목코드(입력)","개입수(입력)"]];
  const seen = new Set();
  unmatched.forEach(item => {
    const skuId = String(item.row[1]||"").trim();
    if (seen.has(skuId)) return;
    seen.add(skuId);
    wsData.push([skuId, String(item.row[2]||""), item.row[3]!=null?String(item.row[3]):"", "", ""]);
  });
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(wsData);
  ws["!cols"] = [{wch:12},{wch:55},{wch:16},{wch:20},{wch:10}];
  XLSX.utils.book_append_sheet(wb, ws, "미매칭");
  const brandName = brand === "floem" ? "플로엠" : "쁘띠팬시";
  XLSX.writeFile(wb, `미매칭목록_${brandName}.xlsx`);
}
        ...
        {step === "result" && processed && (
          <div className="result-wrap">
            ...
            <div className="action-row">
              ...
              {stats.unmatched > 0 && (
                <button className="btn-ghost" onClick={()=>exportUnmatched(processed, brand)}>
                  📋 미매칭 {stats.unmatched}건 내보내기
                </button>
              )}
            </div>
